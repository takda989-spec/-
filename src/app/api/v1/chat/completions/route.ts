...
import { checkEndpointBlock } from "@/server/middleware/endpointBlocker";
import { transformRequestBody } from "@/lib/spoof/modelAliasTransform";
...
export async function POST(request) {
  ...
  // Spoof: Endpoint blocking check
  const blockResult = checkEndpointBlock(request);
  ...
  try {
    ...
    if (parsedBody) {
      // Spoof: Transform model aliases in request (accepts both original and alias)
      parsedBody = transformRequestBody(parsedBody);
