...
import { isEndpointBlocked } from "../../lib/db/endpointBlocks";
...
export async function runAuthzPipeline(
  ...
  // Spoof: Endpoint blocking check (excludes /api/spoof/** management routes and /api/statistic)
  if (!guardedPathname.startsWith("/api/spoof/") && !guardedPathname.startsWith("/api/statistic")) {
    try {
      if (isEndpointBlocked(guardedPathname)) {
        const blocked = NextResponse.json(
          { error: { code: "ACCESS_DENIED", message: "This endpoint has been blocked" } },
          { status: 403 }
        );
        stampRouteResponse(blocked, requestId, classification.routeClass);
        applyCorsHeaders(blocked, request, corsRelaxOrigin);
      ...
      console.error("[endpointBlocker] Check failed:", err);
    }
  }
